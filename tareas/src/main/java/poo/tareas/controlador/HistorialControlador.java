            ...
            return new TableCell<HistorialTarea, String>() {
                ...
                protected void updateItem(String prioridad, boolean empty) {
                    ...
                    if (empty || prioridad == null) {
                        ...
                        // Obtener la tarea del historial de la fila actual
                        HistorialTarea historialTarea = null;
                        if (getTableRow() != null) {
                            historialTarea = (HistorialTarea) getTableRow().getItem();
                        }
                        
                        // Verificar si la tarea estaba vencida
                        boolean tareaVencida = false;
                        if (historialTarea != null && historialTarea.getFechaFinTarea() != null && 
                            !"Completada".equals(historialTarea.getEstadoTarea())) {
                            // Convertir la fecha de finalización a LocalDate
                            LocalDate fechaFinLD = historialTarea.getFechaFinTarea().toInstant()
                                                 .atZone(ZoneId.systemDefault())
                                                 .toLocalDate();
                            // Verificar si la fecha de fin es anterior o igual a la fecha de eliminación
                            LocalDate fechaElimLD = historialTarea.getFechaEliminacion().toInstant()
                                                  .atZone(ZoneId.systemDefault())
                                                  .toLocalDate();
                            tareaVencida = fechaFinLD.isBefore(fechaElimLD) || fechaFinLD.isEqual(fechaElimLD);
                        }
                        
                        // Limpiar estilos anteriores para evitar acumulación
                        getStyleClass().removeAll("prioridad-alta", "prioridad-media", "prioridad-baja", "prioridad-vencida");
                        
                        if (tareaVencida) {
                            // Si estaba vencida, mostrar "Vencida" en lugar de la prioridad
                            setText("Vencida");
                            getStyleClass().add("prioridad-vencida");
                        } else {
